...
// Adaptacao em relacao ao original do unlumen-ui: tipos do React importados
// explicitamente (ComponentProps/ReactNode), exigido pelo tsconfig estrito.
...
// Paleta padrao: azul cobalto (acompanha o token --brand em index.css).
// Passe darkPalette/lightPalette pra usar outra cor sem tocar aqui.
...
export function PixelLiquidBg({
  ...
  pixelSize = 18,
  resolution = 0.4,
  // adaptacao: forca menor (era 8) reage mais suave ao cursor, menos "seco"
  mouseForce = 6,
  cursorSize = 110,
  autoDemo = true,
  children,
  ...
  useEffect(() => {
    ...
    // adaptacao: intensidade menor deixa o movimento automatico do fluido
    // mais lento e liquido, em vez de reagir com solavancos
    mouse.autoIntensity = 1.5;
    ...
    const sim = new FluidSim(gl, mouse, {
      ...
      isBounce: false,
      isViscous: false,
      // adaptacao: mais iteracoes de pressao = fluido menos "quadriculado"/
      // rigido nas bordas, ainda barato com resolution baixa (0.3)
    ...
    const driver = autoDemo
      ? // adaptacao: velocidade menor (era 0.45) pro fluido deslizar em vez
        // de correr atras de um alvo aleatorio
        new AutoDriver(mouse, () => lastInteraction, 0.26, 1200)
      : null;
